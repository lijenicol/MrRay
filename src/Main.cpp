...
struct ScanlineBlock {
	...
	void execute(const Camera& cam, std::shared_ptr<Texture> sb_tex, const Hittable& world, std::shared_ptr<Hittable> lights) {
...
void executeBlock(std::shared_ptr<ScanlineBlock> block, 
	...
	block->execute(*(cam.get()), sb_tex, *(world.get()), lights);
...
void execute(const RenderSettings& renderSettings) {
	...
	std::shared_ptr<Hittable> world = std::make_shared<Sphere>(Vec3(278, 278, 278), 150, red);
	std::shared_ptr<Texture> sb_tex;
	...
	fov = 40;
	aperture = 0.05;
	sb_tex = std::make_shared<SolidColour>(0.1, 0.1, 0.1);

	// Camera
	Vec3 vup(0, 1, 0);
	double focus_dist = (lookFrom - lookAt).length();
	std::shared_ptr<Camera> cam = std::make_shared<Camera>(lookFrom, lookAt, vup, fov, renderSettings.aspectRatio(), aperture, focus_dist);
	...
	std::shared_ptr<RenderSettings> sRenderSettings = std::make_shared<RenderSettings>(renderSettings);
	std::shared_ptr<ScanlineBlock> blocks[renderSettings.threads];
	for (unsigned int blockIndex = 0; blockIndex < renderSettings.threads - 1; blockIndex++) {
		blocks[blockIndex] = std::make_shared<ScanlineBlock>(
			blockIndex, blockIndex * scanlineHeight, scanlineHeight, sRenderSettings);
		remainingLines -= scanlineHeight;
	}

	unsigned int blockIndex =  renderSettings.threads - 1;
	blocks[blockIndex] = std::make_shared<ScanlineBlock>(
		blockIndex, blockIndex * scanlineHeight, remainingLines, sRenderSettings);

	std::thread threads[renderSettings.threads];
	for (unsigned int blockIndex = 0; blockIndex < renderSettings.threads; blockIndex++) {
		threads[blockIndex] = std::thread(
			executeBlock, blocks[blockIndex], cam, sb_tex, world, lights);
	}
	...
	for (const std::shared_ptr<ScanlineBlock>& block : blocks) {
		for (int i = 0; i < block->coloursCount; i++) {
