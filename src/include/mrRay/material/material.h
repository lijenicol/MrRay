...
inline double
schlick(double cosine, double ref_idx)
{
...
public:
    ...
    virtual double
    bsdf(const Ray &r_in, const hit_record &rec, const Ray &r_out) const
    {
        return 0;
    }
...
public:
    ...
    Lambertian(const Colour &a)
        : albedo(std::make_shared<SolidColour>(a))
    {
    }
    ...
    virtual double
    bsdf(const Ray &r_in, const hit_record &rec, const Ray &r_out) const override
    {
        return 1 / pi;
    }
};
