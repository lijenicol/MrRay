...
cornellBox(const RenderSettings &renderSettings)
{
    std::shared_ptr<Lambertian> red
        = std::make_shared<Lambertian>(Colour(.65, .05, .05));
    std::shared_ptr<Lambertian> white
        = std::make_shared<Lambertian>(Colour(.73, .73, .73));
...
main(int argc, char **argv)
{
    ...
    program.add_argument("--threads")
        .scan<'u', unsigned int>()
        .help("Number of parallel threads to run. Default is the maximum "
              "available")
    ...
    program.add_argument("--tilesize")
        ...
        .help("Tile size")
        .default_value(64u);
    program.add_argument("out").help("Output path");
