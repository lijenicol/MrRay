...
HdRayMesh::HdRayMesh(const SdfPath &id)
    : HdMesh(id)
    , _topology()
    , _transform()
    , _points()
...
void
HdRayMesh::Finalize(HdRenderParam *renderParam)
{
}

HdDirtyBits
HdRayMesh::GetInitialDirtyBitsMask() const
{
...
HdDirtyBits
HdRayMesh::_PropagateDirtyBits(HdDirtyBits bits) const
{
    return bits;
}
...
HdRayMesh::Sync(
    ...
    TfToken const   &reprToken)
{
    auto *rayRenderParam = static_cast<HdRayRenderParam*>(renderParam);
    ray::Scene *scene = rayRenderParam->AcquireSceneForEdit();
    ...
    std::shared_ptr<ray::Lambertian> meshMaterial;
    if (HdChangeTracker::IsPrimvarDirty(*dirtyBits, id, HdTokens->displayColor)) {
        ...
        for (auto const& color : value.Get<VtVec3fArray>()) {
            meshMaterial = std::make_shared<ray::Lambertian>(
                ray::Colour(color[0], color[1], color[2]));
            break;
        }
    }
    if (!meshMaterial) {
        meshMaterial = std::make_shared<ray::Lambertian>(
            ray::Colour(0.9, 0.9, 0.9));
    }

    ray::RawMeshInfo rawMeshInfo;
    ...
    _rayMesh = std::make_shared<ray::Mesh>(
        rawMeshInfo,
        ray::Mat4(
