...
void
printBVHProgress(int sortedCount, int totalCount)
{
    int percent_done = (100.0f * (float)((float)sortedCount / totalCount));
    if (percent_done % 10 == 0) {
        std::cerr << "\rBVH Construction " << percent_done << "% Complete"
                  << std::flush;
    }
}

BVHNode::BVHNode(
    std::vector<std::shared_ptr<Hittable>> &objects, size_t start, size_t end,
    double time0, double time1, int depth, int &sorted_counter)
{
    ...
    if (object_span == 1) {
        ...
        left = std::make_shared<BVHNode>(
            objects, start, mid, time0, time1, depth + 1, sorted_counter);
        right = std::make_shared<BVHNode>(
            objects, mid, end, time0, time1, depth + 1, sorted_counter);
    }
...
bool
BVHNode::bounding_box(double time0, double time1, AABB &output_box) const
{
    output_box = box;
    return true;
}
...
bool
BVHNode::hit(const Ray &r, double t_min, double t_max, hit_record &rec) const
{
    if (!box.hit(r, t_min, t_max)) return false;

    bool hit_left = left->hit(r, t_min, t_max, rec);
    bool hit_right = (right != NULL)
