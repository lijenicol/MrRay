...
Colour
NormalTexture::value(double u, double v, const hit_record &rec) const
{
    return (rec.normal + Vec3(1, 1, 1)) / 2.0;
}

Colour
CheckerTexture::value(double u, double v, const hit_record &rec) const
{
    ...
    if (sines < 0) return odd->value(u, v, rec);
...
Colour
ImageTexture::value(double u, double v, const hit_record &rec) const
{
    // Return solid cyan if no image
    if (data == nullptr) return Colour(0.5, 0, 0.5);
    ...
    return Colour(
        colour_scale * pixel[0],
        colour_scale * pixel[1],
